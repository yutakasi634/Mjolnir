...
template<std::size_t N, typename realT,
         template <typename Real> class angle1_potentialT,
         template <typename Real> class angle2_potentialT,
         template <typename Real> class contact_potentialT>
std::vector<std::tuple<std::array<std::size_t, N>, angle1_potentialT<realT>,
                       angle2_potentialT<realT>, contact_potentialT<realT>>>
read_directional_contact_potentials(const toml::value& local)
{
  ...
  using indices_type = std::array<std::size_t, N>;
  using indices_potentials_tuple_type = std::tuple<indices_type, angle1_potentialT<realT>,
                                                   angle2_potentialT<realT>, contact_potentialT<realT>>;
  ...
  for(const auto& item : params)
  {
    ...
    retval.emplace_back(
        std::make_tuple(indices,
            detail::read_local_potential_impl<angle1_potentialT<realT>>::invoke(angle_pot1, env),
            detail::read_local_potential_impl<angle2_potentialT<realT>>::invoke(angle_pot2, env),
            detail::read_local_potential_impl<contact_potentialT<realT>>::invoke(contact_pot, env))
        );
  }
  return retval;
}
